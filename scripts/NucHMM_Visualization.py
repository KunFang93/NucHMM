...
def HMM_matrix_visualization(rawhmmfile,histonelistfile,transmat,markstatemat,mark,outputmark):
    '''Visualization of Transition matrix and mark-state matrix'''

    # matrix colors
    red_white = matplotlib.colors.LinearSegmentedColormap.from_list("", ["#ffffff","#fdc7c7","#ff8686","#fb4444","#ff0000"])
    red_yellow = "YlOrRd"
    red_blue = "coolwarm"
    ...
    emitpic = sns.heatmap(ax=ax, data=out,linewidths=.5,linecolor="black",yticklabels=y_label,annot=out,
                          xticklabels=histone_list, cmap=red_yellow,vmin=0,vmax=bar_max,annot_kws={"fontsize":24,"color":"black"})
