...
def HMM_matrix_visualization(rawhmmfile,histonelistfile,transmat,markstatemat,mark,outputmark,color_pick,emitmat,mark_threshold):
    ...
    transpic = sns.heatmap(ax=ax, data=transout,linewidths=.5,linecolor="black",yticklabels=tick_labels,annot=transout,
                          xticklabels=tick_labels, cmap=matrix_color,vmin=0,vmax=bar_max,annot_kws={"fontsize":24,"color":"black"})
    ...
    # Plot emission probability matrix
    if emitmat is not None:
        emitout = np.array(emit_matrix)
        bar_max = emitout.max()
        fig, ax = plt.subplots()
        # emit_annot_array = transout >=bar_max/4
        emit_labels = []
        for i in range(numstate):
            emit_labels.append('S'+ str(i+1))

        ob_labels = []
        for i in range(np.power(2,numstate)):
            ob_labels.append(str(i))

        # emispic = sns.heatmap(ax=ax, data=emitout,linewidths=.5,linecolor="black",yticklabels=emit_labels,annot=emitout,
        #                       xticklabels=ob_labels, cmap=matrix_color,vmin=0,vmax=bar_max,annot_kws={"fontsize":24,"color":"black"})
        emispic = sns.heatmap(ax=ax, data=emitout,linewidths=.5,linecolor="black", yticklabels=emit_labels,cmap=matrix_color,vmin=0,vmax=bar_max)
        cbar_emis = emispic.collections[0].colorbar
        cbar_emis.ax.tick_params(labelsize=12)
        # for text, show_annot in zip(ax.texts, (element for row in emit_annot_array for element in row)):
        #     text.set_visible(show_annot)
        plt.xticks(fontsize=8,rotation=90)
        plt.yticks(fontsize=10,rotation=0)
        plt.savefig(emitmat,bbox_inches='tight',dpi=300)
