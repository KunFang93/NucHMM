...
def trimmed_name(file):
    ...
    current_name_mainbody = re.findall('\w+(?=.f.*q.gz|.fastq)',current_name)[0]
...
def QC_step(info_dict,threads):
    ...
    for files in inputfiles_list:
        if len(files) == 2:
            ...
            current_R1_trim = trimmed_name(files[0])
            current_R2_trim = trimmed_name(files[1])
            ...
            current_trim = trimmed_name(file)
...
def Mapping_step(fqinfodict,Bindex,B2index,threads,mapq_cutoff):
    ...
    for idx,files in enumerate(inputfileslist):
        if len(files) == 2:
            # paired-end
            out_bam_name = bam_name(files[0],True)
            if readlenlist[idx] >= 50:
                ...
                out_fix_name = out_bam_name[:-4] + '.fixmate.tmp'
                final_out_name = out_bam_name[:-4] + '.final.bam'
                subprocess.call('samtools fixmate -r ' + out_bam_name + ' ' + out_fix_name,shell=True)
                subprocess.call('samtools view -F 1804 -f 2 -b ' + out_fix_name + ' | samtools sort - '+ final_out_name,shell=True)
                subprocess.call('rm ' + out_fix_name,shell=True)
            else:
                subprocess.call('bowtie -p ' + str(threads) + ' ' + Bindexpath + ' -1 ' + files[0] + ' -2 ' + files[1] +
                                ' |samtools view -F 1804 -f 2 -q ' + str(mapq_cutoff) +
                                ' -b - | samtools sort -O BAM -o ' + out_bam_name + ' - ', shell=True)
                out_fix_name = out_bam_name[:-4] + '.fixmate.tmp'
                final_out_name = out_bam_name[:-4] + '.final.bam'
                subprocess.call('samtools fixmate -r ' + out_bam_name + ' ' + out_fix_name,shell=True)
                subprocess.call('samtools view -F 1804 -f 2 -b ' + out_fix_name + ' | samtools sort - '+ final_out_name,shell=True)
                subprocess.call('rm ' + out_fix_name,shell=True)
            aligned_file_list.append(final_out_name)
        else:
            # single-end
            out_bam_name = bam_name(files[0],False)
            if readlenlist[idx] >= 50:
                subprocess.call('bowtie2 -p ' + str(threads) + ' -x ' + B2indexpath + ' -U ' + files[0] +
                                ' |samtools view -F 1804 -q ' + str(mapq_cutoff) + ' -b - | samtools sort -O BAM -o ' +
